...
class ArrayObjectWithGetMethod extends \ArrayObject
{
    ...
    public function __get($key)
    {
        ...
        throw new \InvalidArgumentException($key . ' is not a dynamic property key of ' . TypeDescription::getLocalName($this));
    }
}
...
final class ContainerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function setUp(): void
    {
        ...
        $this->hashReferenceObject = new \ArrayObject($this->hashReference);
        $this->indexedReferenceObject = new \ArrayObject($this->indexedReference);
        $this->sparseIndexedReferenceObject = new \ArrayObject($this->sparseIndexedReference);

        $this->nullValue = null;

        $this->hashReferenceImpl = new ArrayAccessImpl($this->hashReference);
    }
    ...
    public function testKeyValue()
    {
        ...
        foreach ([
            ...
            $this->assertEquals('bar', Container::keyValue($container, 'foo'), 'keyValue of ' . TypeDescription::getLocalName($container));
        }

        $this->assertEquals('default', Container::keyValue($arrayWithGetMethod, 'undefined', 'default'), 'Catch exception when attempting to get object property');

        $hardCodedProperty = Container::keyValue($arrayWithGetMethod, 'hardCodedProperty');
        $this->assertEquals('hard', $hardCodedProperty, 'Initial arrayWithGetMethod::$hardCodedProperty');

        $arrayWithGetMethod['hardCodedProperty'] = 'soft';
        $hardCodedProperty = Container::keyValue($arrayWithGetMethod, 'hardCodedProperty');
        $this->assertEquals('soft', $hardCodedProperty, 'After offsetSet arrayWithGetMethod::$hardCodedProperty');
    }
    ...
    public function testTreeExistsOrSet()
    {
        ...
        foreach ($tests as $label => $test) {
            ...
            $label .= ' - [' . Container::implodeValues($test['keyTree'], ', ') . '] = ' . ($expected ? 'exists' : 'does not exists');
            ...
            $actual = Container::treeExists($container, $keyTree);
            $expected = true;
            $this->assertEquals($expected, $actual, $label . PHP_EOL . 'After set');
        }
    }
    ...
    public function testTreeValue()
    {
        ...
        foreach ($tests as $label => $test) {
            ...
            foreach ([
                ...
                try {
                    $actual = Container::treeValue($container, $keyTree, $dflt, $keySeparator);
                } catch (\Exception $e) {
                    $actual = TypeDescription::getName($e);
                }
                $this->assertEquals($expected, $actual, $label . '(' . TypeDescription::getName($container) . ')');
            }
        }
    }
    ...
    public function testPop()
    {
        ...
        foreach ([
            ...
            foreach ([
                ...
                $label = TypeDescription::getName($container) . ' pass ' . ($pass + 1) . ' (' . Container::implodeValues($container, ', ') . ')';
    ...
    {
        $this->assertEquals(true, Container::valueExists($this->indexedReference, 'two'));
        $this->assertEquals(false, Container::valueExists($this->indexedReference, 'deux'));
    }
    ...
    public function testSetValueBothPropertyAndArrayAccess()
    {
        ...
        $this->assertEquals('foo', $o->hardCodedProperty, 'Property value after Container::setValue');
        $this->assertArrayHasKey('hardCodedProperty', $o, 'ArrayAccess key set after Container::setValue');
        $this->assertEquals('foo', $o['hardCodedProperty'], 'ArrayAccess[hardCodedProperty]');
    }
    ...
    {
        $this->assertEquals(3, Container::count($this->sparseIndexedReference));
    }
    ...
    {
        $this->assertEquals(4, Container::count($this->indexedReferenceObject));
    }

    public function testCountAssociativeArrayObject()
    {
        $this->assertEquals(3, Container::count($this->sparseIndexedReferenceObject));
    }

    public function testCountHashTableObject()
    {
        $this->assertEquals(3, Container::count($this->hashReferenceObject));
    }
    ...
    public function testGetKeysAndValues()
    {
        ...
        $this->assertEquals($expectedKeys, Container::keys($arrayObject), 'Container::keys (ArrayObject)');
        $this->assertEquals($expectedKeys, Container::keys($traversable), 'Container::keys (TraversableImpl)');

        $this->assertEquals($expectedValues, Container::values($array), 'Container::getValue(array)');
        $this->assertEquals($expectedValues, Container::values($arrayObject), 'Container::getValue(ArrayObject)');
        $this->assertEquals($expectedValues, Container::values($traversable), 'Container::getValue(TraversableImpl)');
    ...
    public function testFilter()
    {
        ...
        $this->assertEquals($evenValues, Container::filterValues($series, $oddFilter), 'Filter odd array values');
        $this->assertEquals($evenValues, Container::filterValues($seriesObject, $oddFilter), 'Filter odd ArrayObject values');

        $this->assertEquals($evenKeys, Container::filterKeys($series, $oddFilter), 'Filter odd array keys');
        $this->assertEquals($evenKeys, Container::filterKeys($seriesObject, $oddFilter), 'Filter odd ArrayObject keys');

        $this->assertEquals([
            1 => 2,
            2 => 3
        ], Container::filter($series, function ($k, $v) {
            return $k == 2 || $v == 2;
        }), 'Filter two only');
    }
    ...
    public function testImplodeValueSparse()
    {
        ...
        $evenResult = Container::implode($source, $glue, function ($k, $v) {
            if ($k % 2 == 0)
                return strval($v);
            return false;
        });
        ...
        $evenStringsResult = Container::implode($source, $glue, function ($k, $v) {
            if ($k % 2 == 1)
                return false;
            if ($v === false)
                return false;
        ...
        $oddResult = Container::implode($source, $glue, function ($k, $v) {
            if ($k % 2 == 0)
                return false;
            return strval($v);
        });
        ...
        $stringValuesResult = Container::implodeValues($source, $glue, function ($v) {
            if ($v === false)
                return false;
            return strval($v);
        });
    ...
    {
        $this->assertTrue(Container::isIndexed([]), 'Empty container is indexed by default');
        $this->assertTrue(Container::isAssociative([]), 'Empty container is associative by default');
        $this->assertFalse(Container::isIndexed([], false, false), 'Empty container is indexed if asked');
        $this->assertFalse(Container::isAssociative([], false, false), 'Empty container is associative if asked');
        ...
        $this->assertTrue(Container::isIndexed($explicitIndexes, true), 'Explicitely indexed is indexed (strict)');

        $this->assertTrue(Container::isIndexed($numericStringIndexes), 'numeric string indexes is indexed');
        $this->assertTrue(Container::isIndexed($numericStringIndexes, true), 'numeric string indexes (strict) is still indexed due to PHP auto conversion');

        $this->assertFalse(Container::isIndexed($unordered), 'Unordered is not indexed');
        $this->assertFalse(Container::isIndexed($unordered, true), 'Unordered is not indexed (strict)');

        $this->assertFalse(Container::isAssociative($indexed), 'Indexed is not associative');
        $this->assertFalse(Container::isAssociative($indexed, true), 'Indexed is not associative (strict)');

        $this->assertFalse(Container::isAssociative($explicitIndexes), 'Explicitely indexed is not associative');
        $this->assertFalse(Container::isAssociative($explicitIndexes, true), 'Explicitely indexed is not associative (strict)');

        $this->assertTrue(Container::isAssociative($unordered), 'Unordered is associative');
        $this->assertTrue(Container::isAssociative($unordered, true), 'Unordered is associative (strict)');
    ...
    {
        $this->assertEquals('Three', Container::nthValue([
            'One',
            "Two",
            'Three',
            'Four'
        ...
        $this->assertEquals('Two', Container::nthValue([
            0 => 'One',
            5 => "Two",
            3 => 'Three',
            42 => 'Four'
        ], 1), '2nd value of associative array with integer keys');

        $this->assertEquals(null, Container::nthValue([
            'One',
            'Four'
        ...
        foreach ($tests as $label => $test) {
            ...
            foreach ([
                ...
                $this->assertEquals([
                    $key,
                    $value
                ], Container::first($container, $dflt), $label . ' ' . $type . ' first');

                $this->assertEquals($key, Container::firstKey($container, Container::keyValue($dflt, 0, null)), $label . ' ' . $type . ' firstKey');
                $this->assertEquals($value, Container::firstValue($container, Container::keyValue($dflt, 1, null)), $label . ' ' . $type . ' firstValue');

                $this->assertEquals([
                    $lastKey,
                    $lastValue
                ], Container::last($container, $dflt), $label . ' ' . $type . ' last');
                $this->assertEquals($lastKey, Container::lastKey($container, Container::keyValue($dflt, 0, null)), $label . ' ' . $type . ' last key');
                $this->assertEquals($lastValue, Container::lastValue($container, Container::keyValue($dflt, 1, null)), $label . ' ' . $type . ' last value');
            }
    ...
    public function testMap()
    {
        ...
        $actual = Container::map($input, function ($k, $v) {
            return $k . '.' . \strtoupper($v);
        });

        $this->assertEquals($expected, $actual, 'map without aadditional arguments');
        ...
        $this->assertEquals($expected, $actual, 'mapValues without aadditional arguments');
        ...
        $actual = Container::map($input, function ($k, $v, $suffix) {
            return $k . '.' . \strtoupper($v) . '.' . $suffix;
        }, 'suffix');

        $this->assertEquals($expected, $actual, 'map with aadditional arguments');

        Container::walk($input, function ($k, $v, $suffix) {
            return $k . '.' . \strtoupper($v) . '.' . $suffix;
        }, 'suffix');
    ...
    public function testSorts()
    {
        ...
        foreach ($tests as $f => $test) {
            ...
            $this->assertEquals($arrayExpected, $arrayInput, $f . ' array');

            \call_user_func_array([
                Container::class,
                $f
            ], $objectArgs);
            $this->assertEquals($objectExpected, $objectInput, $f . ' ArrayObject');
        }
    }
    ...
    public function testUniqueValues()
    {
        ...
        foreach ($tests as $label => $test) {
            ...
            $expected = Container::keyValue($test, 'expected', $container);
            ...
            if ($comparer === null && \count($arguments) == 0) {
                ...
                $this->assertEquals($arrayUnique, $actual, $label . ' should returns the same result as array_unique');
            }
    ...
    public function testMerge()
    {
        ...
        $this->assertEquals($expected, $actual, 'Default merge option mimics array_merge');
        ...
        $actual = Container::merge($mapA, $mapB, $listA, $listB, $options);
        ...
        $actual = Container::merge($listA, $listB, $mapA, $mapB, $options);
        $actual = \json_encode($actual, JSON_PRETTY_PRINT);

        $this->assertEquals($expected, $actual, 'Merge / replace list (2)');
    }
    ...
    public function testMergeRecursive()
    {
        ...
        $this->assertEquals($expected, $actual, 'Default recursive merge mimics array_merge_recursive');
    }
